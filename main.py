...
def cargar_dashboard(request: Request, estado: str = Query("activo", regex="^(activo|programado|finalizado|cancelado|todos)$"), db: Session = Depends(get_session)):
    ...
    if estado == "todos":
        eventos = db.exec(select(Evento).order_by(Evento.fecha_evento)).all()
    else:
        eventos = db.exec(select(Evento).where(Evento.estado == estado).order_by(Evento.fecha_evento)).all()
...
def listar_eventos(
    ...
    query = select(Evento).order_by(Evento.fecha_evento)
    if estado:
...
def editar_evento(evento_id: int, data: EventoUpdate, db: Session = Depends(get_session)):
    ...
    evento.id_obra = data.id_obra
    
    # Eliminar relaciones existentes
    db.exec(select(EventoEspecialidad).where(EventoEspecialidad.id_evento == evento_id)).all()
    for rel in db.exec(select(EventoEspecialidad).where(EventoEspecialidad.id_evento == evento_id)).all():
        db.delete(rel)
    
    db.exec(select(EventoExperiencia).where(EventoExperiencia.id_evento == evento_id)).all()
    for rel in db.exec(select(EventoExperiencia).where(EventoExperiencia.id_evento == evento_id)).all():
        db.delete(rel)
...
@app.put("/evento_estado/{evento_id}")
def cambiar_estado_evento(evento_id: int, data: EventoEstado, db: Session = Depends(get_session)):
    evento = db.exec(select(Evento).where(Evento.id_evento == evento_id)).first()
    if not evento:
        return JSONResponse(status_code=404, content={"message": "Evento no encontrado"})
    ...
    db.commit()
    return {"message": f"Estado del evento actualizado a '{data.estado}'"}

@app.get("/evento_detalle/{evento_id}")
def obtener_evento_detalle(evento_id: int, db: Session = Depends(get_session)):
    evento = db.exec(select(Evento).where(Evento.id_evento == evento_id)).first()
    if not evento:
        return JSONResponse(status_code=404, content={"message": "Evento no encontrado"})
    
    modulos = db.exec(
        select(Especialidad.id_especialidad, Especialidad.nombre_especialidad)
        .join(EventoEspecialidad, EventoEspecialidad.id_especialidad == Especialidad.id_especialidad)
        .where(EventoEspecialidad.id_evento == evento_id)
    ).all()
    
    experiencias = db.exec(
        select(Experiencia.id_experiencia, Experiencia.nombre_experiencia)
        .join(EventoExperiencia, EventoExperiencia.id_experiencia == Experiencia.id_experiencia)
    ...
    return {
        ...
        "id_obra": evento.id_obra,
        "estado": evento.estado,
        "modulos": [{"id": m[0], "nombre": m[1]} for m in modulos],
